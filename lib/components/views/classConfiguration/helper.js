...
export var sortAllTopics = function (classInstances) {
    for (var schoolYearId in classInstances) {
        for (var subjectId in classInstances[schoolYearId].subjects) {
            for (var classLevelId in classInstances[schoolYearId].subjects[subjectId].classLevels) {
                for (var classId in classInstances[schoolYearId].subjects[subjectId]
                    .classLevels[classLevelId].classes) {
                    var respectiveClass = classInstances[schoolYearId].subjects[subjectId].classLevels[classLevelId].classes[classId];
                    respectiveClass.topics = respectiveClass.topics.sort(function (topic1, topic2) { return topic1.startIndex - topic2.startIndex; });
                }
            }
        }
    }
